...
<script>
import { ImagesLoader } from '@/utils/tool'
export default {
  ...
  methods: {
    ...
    handleMouseDown(event) {
      if (!this.disabled) {
        this.mouse.savedPositionY = event.pageY
    ...
    handleTouchStart(event) {
      ...
      if (!this.disabled) {
        this.mouse.savedPositionY = event.touches[0].pageY
    ...
    handleTouchMove(event) {
      event.preventDefault()
      this.handleMovement(event.touches[0].pageX, event.touches[0].pageY)
    },
    handleMouseMove(event) {
      this.handleMovement(event.pageX, event.pageY)
    },
    handleMovement(currentPosition, YcurrentPosition) {
      if (this.mouse.isMoving) {
        this.mouse.currentPositionX = currentPosition
        this.mouse.currentPositionY = YcurrentPosition
        const distance = this.mouse.currentPositionX - this.mouse.savedPositionX
        const Ydistance = YcurrentPosition - this.mouse.savedPositionY
        ...
        if (Math.abs(Ydistance) > this.speed) {
          this.mouse.savedPositionY = this.mouse.currentPositionY
          if (
            (Ydistance > 0 && !this.reverse) ||
            (Ydistance < 0 && this.reverse)
          ) {
            this.slideToRight()
          } else {
